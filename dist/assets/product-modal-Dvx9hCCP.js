import{a as e,i as t,o as n,s as r,t as i}from"./cart-utils-Cg7pFG_6.js";function a(e){return e?String(e).replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}function o(e){let t=e.id||`product-${Date.now()}`,n=a(e.querySelector(`.product-card__name`)?.textContent?.trim()||`Producto`),r=a(e.querySelector(`.product-card__shop`)?.textContent?.trim()?.replace(/^\s*/,``)||`Tienda`),i=a(e.querySelector(`.product-card__price`)?.textContent||`$0`),o=a(e.querySelector(`.product-card__price-old`)?.textContent||``),s=a(e.querySelector(`.product-card__discount`)?.textContent||``),c=encodeURI(e.querySelector(`.product-card__image img`)?.getAttribute(`src`)||``),l=a(e.querySelector(`.product-card__image img`)?.getAttribute(`alt`)||n),u=a(e.querySelector(`.product-card__shipping`)?.textContent?.trim()||``),d=e.querySelector(`.product-card__stars`),f=d?.querySelectorAll(`.fa-star:not(.empty)`).length||0,p=d?.querySelectorAll(`.fa-star-half-stroke`).length||0,m=d?.querySelectorAll(`.fa-star.empty, .fa-regular.fa-star.empty`).length||0,h=e.querySelector(`.product-card__rating-count`)?.textContent?.replace(/[()]/g,``)||`0`,g=(e.dataset.category||``).split(` `),_=e.querySelector(`.product-card__badge`);return{id:t,name:n,shop:r,priceText:i,priceOldText:o,discountText:s,imgSrc:c,imgAlt:l,shippingText:u,fullStars:f,halfStars:p,emptyStars:m,ratingCount:h,categories:g,badgeText:_?.textContent?.trim()||``,badgeType:_?.classList.contains(`product-card__badge--envio`)?`envio`:_?.classList.contains(`product-card__badge--oferta`)?`descuento`:``,stock:t.split(``).reduce((e,t)=>e+t.charCodeAt(0),0)%40+5}}function s(e,t,n){let r=``;for(let t=0;t<e;t++)r+=`<i class="fa-solid fa-star"></i>`;for(let e=0;e<t;e++)r+=`<i class="fa-solid fa-star-half-stroke"></i>`;let i=5-e-t;for(let e=0;e<i;e++)r+=`<i class="fa-regular fa-star empty"></i>`;return r}function c(e){return e<=10?{text:`¡Últimas ${e} unidades!`,cssClass:`low`,fillClass:`pm-stock__fill--low`}:e<=25?{text:`Quedan ${e} unidades`,cssClass:`ok`,fillClass:`pm-stock__fill--mid`}:{text:`Disponible (${e} unidades)`,cssClass:`ok`,fillClass:`pm-stock__fill--high`}}function l(e,t){let n=document.querySelectorAll(`.product-card`),r=[];return n.forEach(n=>{if(n.id===e)return;let i=(n.dataset.category||``).split(` `);t.some(e=>e&&i.includes(e))&&r.push(n)}),r.length<4&&n.forEach(t=>{t.id!==e&&!r.includes(t)&&r.length<6&&r.push(t)}),r.slice(0,6)}function u(e){let t=c(e.stock),n=e.shop.replace(/^[\s\W]*/,``).charAt(0).toUpperCase();e.shop.replace(/^[\s]*/,``).replace(/^\s*\S+\s*/,``);let r=l(e.id,e.categories),i=``;r.forEach(e=>{let t=e.querySelector(`.product-card__name`)?.textContent?.trim()||``,n=e.querySelector(`.product-card__price`)?.textContent||``,r=e.querySelector(`.product-card__image img`)?.getAttribute(`src`)||``,a=e.id||``;i+=`
